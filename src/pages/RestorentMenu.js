...
const RestorentMenu = () => {
  ...
  return (
    <div className="mx-[150px]">
      <h1 className="text-3xl font-bold my-4">{restaurantInfo?.name}</h1>
      <p className="mb-2">Rating: {restaurantInfo?.avgRating} </p>
      <p className="mb-6">
        {restaurantInfo?.areaName}, {restaurantInfo?.city},{" "}
        {restaurantInfo?.costForTwoMessage}
      </p>
      <div>
        <h3 className="text-2xl font-semibold mb-4">{menuItems?.title}</h3>
        {menuItems.map((item, index) => (
          <div
            key={`${item?.card?.info?.id}-${index}`}
            className="flex border-b border-gray-300 px-5 py-3"
          >
